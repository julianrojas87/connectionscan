...
class CSA {
     ...
     */
    constructor(connectionProvider, interstopDistanceCalculator, boundsCalculator, enableISDCaches, connectionEmitter) {
    ...
    async calculateProfile(source, target, maxLegs, minimumDepartureTime) {
        ...
        if (departureTimeUpperBound !== null) {
            ...
            //console.log(this.connectionProvider.store.documentIri);
            while (connection !== null && connection.departureTime >= departureTimeBounds.lower) {
                ...
                if (this.discover(connection)) {
                    // This connection is taken into account for route calculation so emit an event
    ...
    async getJourneys(source, target, minimumDepartureTime, maxLegs, recalculateProfile = false, emitter) {
        let filteredProfile = await this.calculateProfile(source, target, maxLegs, minimumDepartureTime);

        if(filteredProfile !== null) {
            return await this.journeyExtractor.extractJourneys(filteredProfile, source, target, minimumDepartureTime, emitter);
        } else {
            emitter.emit('end');
            return;
        }
    ...
    initProfileEntry(stop) {
        this.profile[stop] = [{
            departureTime: Infinity, arrivalTimes: Array(this.maxLegs).fill(Infinity),
